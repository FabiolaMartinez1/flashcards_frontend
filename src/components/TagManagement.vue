<template>
  <div class="d-flex dropdown ms-auto">
    ...
    <ul class="dropdown-menu p-2" style="width: auto; padding: 1rem" @click.stop>
      <h5>Etiquetas <i v-if="!flagEditableMode" class="tag bi bi-pencil-square fs-4" @click="editableMode"></i></h5>
      <hr>
      <div v-if="!flagEditableMode" class="d-flex flex-wrap">
...
<!--         
        ...
        <div class="d-flex align-items-center me-2 mb-2">
          <input 
            v-if="isAddingTag"
            class="form-control form-control-lg editable" 
            v-model="newTagName"
            @keyup.enter="addTag"
            @blur="isAddingTag = false"
          />
          <button v-else class="btn btn-primary" @click="isAddingTag = true">+ Nueva etiqueta</button>
        </div> -->
      </div>
